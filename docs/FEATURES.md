...
| 3.10 | WebSocketチケット認証 | 30秒有効ワンタイムJWT + fingerprinting | ✅ |
| 3.11 | ターミナルリサイズ | cols/rows動的調整 | ✅ |
| 3.12 | PTYセッション維持 | 切断後5分間セッション保持、再接続で復帰 | ✅ |
| 3.13 | コンテナAgent | Goバイナリ、Flyプライベートネットワーク:9000 | ✅ |
| 3.14 | ファイルブラウザUI | リスト/グリッド表示、breadcrumb、upload、mkdir | ✅ |
| 3.15 | ターミナルUI | フルスクリーン、接続ステータス、マシン起動ボタン | ✅ |
...
| 7.4 | サービス一覧UI | ステータス表示、ドメインリンク、デプロイモーダル | ✅ |
...
| 11.2 | ダークモード/ライトモード切り替え | デフォルトlight、ユーザートグル、localStorage永続化 | ✅ |
| 11.3 | マシンステータスウィジェット | running/sleeping/stopped | ✅ |
...
| # | 機能 | 詳細 | ステータス |
|---|------|------|-----------|
| 12.1 | レート制限 | Tier別リクエスト制限、User双方 | ✅ |
| 12.2 | APIキー暗号化 | AES-256-CBC、キー長検証済 | ✅ |
...
| 12.6 | マシンイメージ更新 | Dockerfile作成済 | ✅ |
| 12.7 | 構造化ログ | JSON、request_id/user_id付き | ✅ |
...
| # | 機能 | 詳細 | ステータス |
|---|------|------|-----------|
| 16.1 | Electronベース | Mac/Windows/Linux対応 | ✅ |
| 16.2 | システムトレイ常駐 | 一時停止/再開/録音/設定/終了 | ✅ |
| 16.3 | 画面キャプチャ + OCR | desktopCapturer → Tesseract.js → テキスト | ✅ |
| 16.4 | フレーム差分検出 | Jaccard類似度で変化検出、無変化時はスキップ | ✅ |
...
| Phase | 合計 | ✅ 実装済 | 🔧 設計済 | ◻ 未着手 |
|-------|------|----------|----------|----------|
| Phase 1 (MVP) | 62 | 61 | 0 | 1 (Gemini) |
| Phase 2 (差別化) | 25 | 21 | 1 | 3 |
| Phase 3 (Growth) | 36 | 28 | 2 | 6 |
| **合計** | **123** | **110** | **3** | **10** |
